...
from sqp_project.automtmm.retrieve_results import read_maxima
...
class LisrelInput:
            ...
            if matforms[matname]['Form'] == 'DI': # or vec
                ...
                for igrp in range(ngroups):
                    ...
                    if len(numbers)/ngroups != matlen and \
                        ...
                        matforms[matname]['Form'] = 'DI' # change it to DI
                        # NOTE: this does not work if the form of the matrix
                        #  is different across groups! Will raise an error
                        #  on trying to symmetrize a diagonal matrix below
    ...
    def run_lisrel(self, temp_path = ''):
        ...
        cmd = [exstr, '"%s"' % os.path.abspath(self.path),  'OUT']  
    ...
    def get_derivs(self, groupnum = 2, 
        ...
        for param in free_params.values():
            try:
                ...
                print "param: ", param
                print "index: ", read_maxima.paramdict[param.lower()]
                print "deriv length: ", len(derivs)
                print "paramdict: ", read_maxima.paramdict
                raise e
    ...
    def get_var_standardized(self, path='temp', 
        ...
        # DEBUG: quick fix for round 4, remove all elements referring to >9 variables
        select = (('ly 1 1', 'ly 2 2', 'ly 3 3', 'ga 1 1', 'ga 2 2', 'ga 3 3',
            'ly 4 4', 'ly 5 5', 'ly 6 6', 'ga 4 1', 'ga 5 2', 'ga 6 3' ), 
            ('ly 7 7', 'ly 8 8', 'ly 9 9', 'ga 7 1', 'ga 8 2', 'ga 9 3'), )
        ...
        for groupnum in range(num_groups): 
            ...
            for i, val in np.ndenumerate(D):
                try:
                    ...
                    # Print relevant info upon error
                    print val
                    print 'ga:\n\t', ga, '\n'
                    print 'ly:\n\t', ly, '\n'
                    print 'ph:\n\t', ph, '\n'
            ...
            try:
                D_stacked[:,groupnum*prev_cols:((groupnum*prev_cols)+D_e.shape[1])] = D_e
            except Exception, e:
                print "Group", groupnum+1, " of ", num_groups
                print "prev_cols is  ", prev_cols
                print "shape of D_e: ", D_e.shape
                print "shape of D_stacked: ", D_stacked.shape
                print D_e
                raise e
