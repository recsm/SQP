...
from sqp_project.sqp.models import Characteristic, Label, CharacteristicSet, Branch
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        """Add the chars 'comp_assist', 'interviewer', 'visual', and 'position' to char set 'All Characteristics' """
        
        def getch(shnam): # quick hack
            return Characteristic.objects.get(short_name = shnam)
        
        char_set = CharacteristicSet.objects.get(pk=3)
        
        chars = {'comp_assist'  :  [{'name'  : 'Yes',                    'code' : '1',   'branch_to_char' : 'interviewer' ,  'compute' : 0}, 
                                    {'name'  : 'No',                     'code' : '0',   'branch_to_char' : 'interviewer',   'compute' : 0}],
                'interviewer'  :   [{'name'  : 'Yes',                    'code' : '1',   'branch_to_char' : 'visual',        'compute' : 0}, 
                                    {'name'  : 'No',                     'code' : '0',   'branch_to_char' : 'visual',        'compute' : 0}],
                'visual'        :   [{'name' : 'Visual',                 'code' : '1',  'branch_to_char' : 'position',      'compute' : 0}, 
                                    {'name'  : 'Oral',                   'code' : '0',  'branch_to_char' : 'position',      'compute' : 0}],
                'position'  :       [{'name' : 'True',                   'code' : '',    'branch_to_char' :  None,           'compute' : 1}],
                 }
        
        #First we make all the groovy labels
        for shnam in chars.keys():
            char = getch(shnam)
            for label_def in chars[shnam]:
                #Create a label for each label in the definition
                label  = Label(name             =   label_def['name'], 
                               code             =   label_def['code'],
                               characteristic   =   char,
                               compute          =   label_def['compute'])
                label.save();
            
        #Next we create all of the extra groovy branches
        for shnam in chars.keys():
            char = getch(shnam)
            for label_def in chars[shnam]:
                
                if label_def['branch_to_char'] is None:
                    continue
                
                #Create a label for each label in the definition
                label = Label.objects.get(name =   label_def['name'], 
                               code             =   label_def['code'],
                               characteristic   =   char)
                
                to_char = getch(label_def['branch_to_char'])
                branch = Branch(label=label, to_characteristic=to_char)
                branch.save()
                
                #Add the branch to our characteristic set
                char_set.branches.add(branch)
        
        #Add in the extra branches to comp_assist from the existing labels
        #We need branches from showcard used (no), picture on the showcard (no), picture on the showcard (yes)
        from_labels = []
        usedshowcard = getch('usedshowcard')
        from_labels.append(Label.objects.get(characteristic=usedshowcard, code='0'))
        showc_pict = getch('showc_pict')
        from_labels.append(Label.objects.get(characteristic=showc_pict, code='0'))
        from_labels.append(Label.objects.get(characteristic=showc_pict, code='1'))
        to_char = getch('comp_assist')
        
        for flabel in from_labels:
            branch = Branch(label=flabel, to_characteristic=to_char)
            branch.save()
            #Add the branch to our characteristic set
            char_set.branches.add(branch)
