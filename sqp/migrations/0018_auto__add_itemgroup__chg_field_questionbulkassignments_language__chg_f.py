...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        """A bit more custom sql for this migration"""
        
        db.execute("""
        CREATE TABLE `sqp_itemgroup_items` (
            `id` integer AUTO_INCREMENT NOT NULL PRIMARY KEY,
            `itemgroup_id` integer NOT NULL,
            `item_id` int(11) UNSIGNED NOT NULL,
            UNIQUE (`itemgroup_id`, `item_id`));   """);
        
        db.execute("""
        ALTER TABLE `sqp_itemgroup_items` ADD CONSTRAINT `item_id_refs_id_5ec13880` 
        FOREIGN KEY   (`item_id`) REFERENCES `sqp_item` (`id`);  """)
        
        db.execute("""
        CREATE TABLE `sqp_itemgroup` (
            `id` integer AUTO_INCREMENT NOT NULL PRIMARY KEY,
            `name` varchar(80) NOT NULL);  """)
        
        db.execute(""" 
        ALTER TABLE `sqp_itemgroup_items` ADD CONSTRAINT `itemgroup_id_refs_id_1412af7c`
        FOREIGN KEY (`itemgroup_id`) REFERENCES `sqp_itemgroup` (`id`);  """)
     

    def backwards(self, orm):
        
        # Deleting model 'ItemGroup'
        db.delete_table('sqp_itemgroup')

        # Removing M2M table for field items on 'ItemGroup'
        db.delete_table('sqp_itemgroup_items')
    ...
    models = {
        ...
        'sqp.questionbulkassignments': {
            ...
            'study': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['sqp.Study']", 'null': 'True', 'blank': 'True'}),
