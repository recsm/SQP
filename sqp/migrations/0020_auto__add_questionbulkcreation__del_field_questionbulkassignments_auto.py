...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        ##More custom sql to avoid 1005 innodb foregin key restraint errors
        #In this case it is the latin 1 encoding of the country id that fixes the error
        db.execute_many("""
        CREATE TABLE `sqp_questionbulkcreation_created_questions` (
            `id` integer AUTO_INCREMENT NOT NULL PRIMARY KEY,
            `questionbulkcreation_id` integer NOT NULL,
            `question_id` integer NOT NULL,
            UNIQUE (`questionbulkcreation_id`, `question_id`)
        )
        ;
        ALTER TABLE `sqp_questionbulkcreation_created_questions` ADD CONSTRAINT 
        `question_id_refs_id_30c1e611` FOREIGN KEY (`question_id`) REFERENCES `sqp_question` (`id`);
        
        CREATE TABLE `sqp_questionbulkcreation` (
            `id` integer AUTO_INCREMENT NOT NULL PRIMARY KEY,
        ...
        ALTER TABLE `sqp_questionbulkcreation` ADD CONSTRAINT `item_group_id_refs_id_7cdc9bab` 
        FOREIGN KEY (`item_group_id`) REFERENCES `sqp_itemgroup` (`id`);
        ALTER TABLE `sqp_questionbulkcreation` ADD CONSTRAINT `country_id_refs_iso_27d77fd9` 
        FOREIGN KEY (`country_id`) REFERENCES `sqp_country` (`iso`);
        ALTER TABLE `sqp_questionbulkcreation` ADD CONSTRAINT `language_id_refs_id_6afb7f20` 
        FOREIGN KEY (`language_id`) REFERENCES `sqp_language` (`id`);
        ALTER TABLE `sqp_questionbulkcreation_created_questions` ADD CONSTRAINT 
        `questionbulkcreation_id_refs_id_41e9b9e` FOREIGN KEY (`questionbulkcreation_id`) 
        REFERENCES `sqp_questionbulkcreation` (`id`);
        
        
        ALTER TABLE `sqp_questionbulkcreation` ADD UNIQUE `unique_creation` ( `item_group_id` , `country_id` , `language_id` );
        ALTER TABLE `sqp_questionbulkassignments` ADD UNIQUE `unique_assignment` ( `item_group_id` , `country_id` , `language_id` ); 
        ALTER TABLE `sqp`.`sqp_userquestion` ADD UNIQUE `unique_user_assignment` ( `user_id` , `question_id` );  
    ...
    def backwards(self, orm):
        ...
        # Deleting model 'QuestionBulkCreation'
        db.delete_table('sqp_questionbulkcreation')

        # Removing M2M table for field created_questions on 'QuestionBulkCreation'
        db.delete_table('sqp_questionbulkcreation_created_questions')

        # Adding field 'QuestionBulkAssignments.auto_create_questions'
        db.add_column('sqp_questionbulkassignments', 'auto_create_questions', self.gf('django.db.models.fields.BooleanField')(default=False), keep_default=False)
