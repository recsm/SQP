...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Query 1 - Empty out all previous codings
        # Query 2 - Empty out completion records
        # Query 3 - Empty out previous coding suggestions
        # We leave any of the user's own codings on the server for the moment for non ESS stuff, where the user id >= 129
        # Query 4 - Delete the 27 duplicate questions with gb code since they have no text or codings
        # Query 4 - Delete the 27 duplicate questions from the SS Source Country since they are duplicates of SF
        # Query 5 - Move all the questions in the db currently marked as source over to GB
        # Query 6 - Delete the source countries SF and SS from the country list
        # Query 7 - Make sure the GB country is called United Kingdom
        source_sql = """
        
            DELETE FROM sqp_coding;
            DELETE FROM sqp_completion;

            TRUNCATE sqp_codingsuggestion;
            
            DELETE FROM sqp_question 
            WHERE country_id  IN ('GB', 'SS');
        
            UPDATE sqp_question 
            SET country_id = 'GB' 
            WHERE country_id = 'SF';
        
            DELETE FROM sqp_country 
            WHERE iso IN ('SS', 'SF');
        ...
        db.execute_many(source_sql)
        
        ##Rerun the mtmm imports to get the correct db mtmm values
